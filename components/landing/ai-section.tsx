...
const sdkFeatures = [
  {
    title: "AI SDK Core",
    description: "A unified API for generating text, structured objects, embeddings, and tool calls with LLMs.",
  },
  {
    title: "AI SDK UI",
    description: "A set of framework-agnostic hooks for quickly building chat, completion, and assistant frontends.",
  },
  {
    title: "AI SDK RSC",
    description: "A library to stream data and generative UI components with React Server Components (RSC).",
  },
]
...
function AIVisual({ type }: { type: number }) {
  if (type === 0) {
    return (
      <svg viewBox="0 0 200 120" className="w-full h-full p-4">
        {[0, 1, 2, 3, 4].map((i) => (
          <g key={i}>
            <rect x={10} y={10 + i * 22} width={16} height={16} rx={4} fill="rgba(255,255,255,0.2)" />
            <path
        ...
        {[0, 1, 2].map((i) => (
          <rect key={`out-${i}`} x={160} y={30 + i * 30} width={30} height={20} rx={4} fill="rgba(255,255,255,0.2)" />
        ))}
      </svg>
    )
  }
    ...
    return (
      <div className="w-full h-full p-4 flex flex-col gap-2">
        ...
        <div className="bg-zinc-800 rounded p-2 text-xs text-gray-400">
          ...
          <span className="ml-2 px-1.5 py-0.5 bg-cyan-600 text-white rounded text-[10px]">Query</span>
        </div>
  ...
  return (
    <svg viewBox="0 0 200 120" className="w-full h-full">
      <ellipse cx={100} cy={60} rx={80} ry={50} stroke="rgba(255,255,255,0.2)" strokeWidth="1" fill="none" />
      <ellipse cx={100} cy={60} rx={80} ry={30} stroke="rgba(255,255,255,0.2)" strokeWidth="1" fill="none" />
      <ellipse cx={100} cy={60} rx={80} ry={10} stroke="rgba(255,255,255,0.2)" strokeWidth="1" fill="none" />
      <line x1={20} y1={60} x2={180} y2={60} stroke="rgba(255,255,255,0.2)" strokeWidth="1" />
      <line x1={100} y1={10} x2={100} y2={110} stroke="rgba(255,255,255,0.2)" strokeWidth="1" />
      {[
        [60, 40],
        [140, 35],
        [80, 80],
        ...
        <circle key={i} cx={x} cy={y} r={3} fill={i % 2 ? "#22D3EE" : "#A855F7"} opacity={0.8} />
      ))}
    </svg>
  )
}
