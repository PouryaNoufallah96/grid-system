...
const testimonials = [
  ...
  {
    quote: "Now we can invest in building new features instead of managing infrastructure.",
    company: "neo",
  },
  {
    quote: "By far my favorite Vercel feature is the ability to connect as many domains as I need to a single project.",
    company: "super",
  },
  {
    quote:
        ...
        <Grid columns={3} rows={3}>
          ...
          {testimonials.map((item, index) => {
            ...
            return (
              ...
              >
                {item.quote ? (
                  <p className="text-sm text-gray-400 leading-relaxed">&quot;{item.quote}&quot;</p>
                ) : (
                  <p className="text-sm text-gray-400 leading-relaxed">
                    {item.company}{" "}
                    <span className="text-white font-medium">
                      {item.stat ? `${item.description} ${item.stat}` : item.description}
                    </span>{" "}
                    with Vercel.
                  </p>
                )}
                ...
                <div className="mt-8 flex items-center justify-between">
                  <span className="text-lg font-bold tracking-tight text-white">{item.company}</span>
